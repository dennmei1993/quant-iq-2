  ...
  return (
    <div>
      <div style={{ marginBottom: "1.5rem" }}>
        <h1 style={{ color: "var(--cream)", fontFamily: "serif", fontSize: "1.8rem", marginBottom: "0.25rem" }}>Investment themes</h1>
        <p style={{ color: "rgba(232,226,217,0.35)", fontSize: "0.82rem" }}>AI-generated investment theses across 1m, 3m, and 6m horizons</p>
      </div>
...
function ThemeCard({ theme }: { theme: Record<string, unknown> }) {
  const momentum   = String(theme.momentum ?? "neutral");
  const conviction = Number(theme.conviction ?? 0);
  const mColor     = MOMENTUM_COLOR[momentum] ?? "var(--signal-neut)";
  const tickers    = (theme.candidate_tickers as string[] | null) ?? [];
    ...
    <div style={{ background: "var(--navy2)", border: "1px solid var(--dash-border)", borderRadius: 10, padding: "1.2rem 1.3rem", marginBottom: "1rem" }}>
      ...
      <div style={{ marginBottom: "0.6rem" }}>
        <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "0.3rem" }}>
          <span style={{ fontSize: "0.68rem", color: mColor }}>{MOMENTUM_LABEL[momentum]}</span>
      ...
      {theme.expires_at && (
        <div style={{ fontSize: "0.65rem", color: "rgba(232,226,217,0.2)", marginTop: "0.75rem" }}>
          Expires {new Date(String(theme.expires_at)).toLocaleDateString("en-US", { month: "short", day: "numeric" })}
        </div>
      )}
    </div>
  );
