...
const STYLE_META: Record<StyleFocus, { label: string; icon: string; desc: string; color: string }> = {
  growth:      { label: "Growth",      icon: "↗", desc: "High-conviction momentum picks, accepts volatility", color: "var(--signal-bull)" },
  balanced:    { label: "Balanced",    icon: "◈", desc: "Mix of growth and stability across sectors",         color: "var(--color-info)" },
  defensive:   { label: "Defensive",   icon: "◇", desc: "Low-beta, stable earnings, capital preservation",   color: "var(--signal-neut)" },
  income:      { label: "Income",      icon: "◎", desc: "Dividend-focused, yield generation, REITs/ETFs",    color: "var(--gold)" },
  speculative: { label: "Speculative", icon: "◉", desc: "High risk/reward, crypto, small-cap, thematic",    color: "var(--signal-bear)" },
};
...
function HeatGauge({ score, size = "md" }: { score: number; size?: "sm" | "md" }) {
  ...
  const color = score >= 3 ? "var(--signal-bull)" : score >= 0 ? "var(--signal-neut)" : score >= -3 ? "var(--signal-neut)" : "var(--signal-bear)";
  const h = size === "sm" ? 4 : 6;
  return (
    <div style={{ display: "flex", alignItems: "center", gap: "0.5rem", flex: 1 }}>
      <div style={{ flex: 1, height: h, background: "var(--border)", borderRadius: h, overflow: "hidden", position: "relative" }}>
...
function MacroPanel({ macro, strategy, mode }: { macro: MacroScore[]; strategy: Strategy | null; mode: BuildMode }) {
  ...
  const overallColor = overallScore >= 2 ? "var(--signal-bull)" : overallScore >= 0 ? "var(--signal-neut)" : overallScore >= -2 ? "var(--signal-neut)" : "var(--signal-bear)";
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: "0.6rem" }}>
        {macro.map(m => (
          <div key={m.aspect} style={{ display: "grid", gridTemplateColumns: "7rem 1fr", gap: "0.75rem", alignItems: "center" }}>
            <div>
              <div style={{ fontSize: "0.68rem", fontWeight: 600, color: "var(--text-2)", textTransform: "capitalize" }}>{m.aspect}</div>
              <div style={{ fontSize: "0.6rem", color: T.dim, textTransform: "capitalize" }}>{m.direction}</div>
            </div>
            <div style={{ display: "flex", flexDirection: "column", gap: "0.2rem" }}>
              <HeatGauge score={m.score} />
              <div style={{ fontSize: "0.62rem", color: "var(--text-4)", lineHeight: 1.4 }}>{m.commentary}</div>
            </div>
          </div>
        ))}
      </div>
      {strategy && mode === "data" && (
        <div style={{ marginTop: "0.9rem", padding: "0.65rem 0.85rem", background: "rgba(180,83,9,0.05)", border: "1px solid rgba(180,83,9,0.15)", borderRadius: 7, fontSize: "0.72rem", color: T.dim, lineHeight: 1.5 }}>
...
function SignalStrengthBar({ label, value, color }: { label: string; value: number | null; color: string }) {
  if (value == null) return (
    <div style={{ display: "flex", alignItems: "center", gap: "0.4rem" }}>
      ...
      <span style={{ fontSize: "0.65rem", color: "var(--text-4)" }}>—</span>
    </div>
  );
  return (
    <div style={{ display: "flex", alignItems: "center", gap: "0.4rem" }}>
...
function LlmProviderToggle({ provider, modelId, onChange }: { provider: LlmProvider; modelId: string; onChange: (provider: LlmProvider, modelId: string) => void }) {
  return (
    <div style={{ background: "var(--bg-subtle)", border: "1px solid var(--border)", borderRadius: 9, padding: "0.85rem 1rem", marginBottom: "1rem", display: "flex", flexDirection: "column" as const, gap: "0.75rem" }}>
      <div style={{ fontSize: "0.62rem", color: "var(--text-4)", textTransform: "uppercase", letterSpacing: "0.08em" }}>LLM Provider</div>
        ...
        {(Object.keys(LLM_PROVIDER_META) as LlmProvider[]).map(p => {
          ...
          return (
            <button key={p} onClick={() => onChange(p, meta.models[0].id)}
              style={{ padding: "0.4rem 1rem", background: active ? `${meta.color}15` : "var(--border-subtle)", border: `1.5px solid ${active ? `${meta.color}50` : "var(--border)"}`, color: active ? meta.color : "var(--text-3)", borderRadius: 7, fontSize: "0.82rem", fontWeight: active ? 700 : 400, cursor: "pointer", transition: "all 0.15s", display: "flex", alignItems: "center", gap: "0.4rem" }}>
        ...
        {LLM_PROVIDER_META[provider].models.map(m => {
          ...
          return (
            <button key={m.id} onClick={() => onChange(provider, m.id)}
              style={{ padding: "0.3rem 0.8rem", background: active ? `${color}12` : "transparent", border: `1px solid ${active ? `${color}40` : "var(--border)"}`, color: active ? color : "var(--text-4)", borderRadius: 5, fontSize: "0.75rem", fontWeight: active ? 600 : 400, cursor: "pointer", transition: "all 0.15s", display: "flex", flexDirection: "column" as const, alignItems: "flex-start", gap: "0.1rem" }}>
              <span>{m.label}</span>
        ...
        return (
          <div key={step} style={{ display: "flex", alignItems: "center" }}>
            <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: "0.3rem" }}>
              <div style={{ width: 32, height: 32, borderRadius: "50%", background: done ? T.gold : active ? "rgba(180,83,9,0.15)" : "var(--border-subtle)", border: `1.5px solid ${done ? T.gold : active ? "rgba(180,83,9,0.6)" : T.border}`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: "0.8rem", fontWeight: 700, color: done ? "var(--bg)" : active ? T.gold : T.dim, transition: "all 0.3s" }}>
                {done ? "✓" : step}
              </div>
            ...
            {i < labels.length - 1 && (
              <div style={{ width: 60, height: 1, background: done ? "rgba(180,83,9,0.4)" : T.border, margin: "0 0.4rem 1.4rem" }} />
            )}
          </div>
        );
      })}
...
function Spinner() {
  return (
    <div style={{ display: "flex", alignItems: "center", gap: "0.6rem", color: T.dim, fontSize: "0.85rem" }}>
      <div style={{ width: 16, height: 16, border: "2px solid rgba(180,83,9,0.2)", borderTopColor: T.gold, borderRadius: "50%", animation: "spin 0.8s linear infinite" }} />
...
function PillToggle({ active, onClick, children, color }: { active: boolean; onClick: () => void; children: React.ReactNode; color?: string }) {
  ...
  return (
    <button onClick={onClick} style={{ padding: "0.3rem 0.8rem", background: active ? `${c}20` : "var(--border-subtle)", border: `1px solid ${active ? `${c}60` : T.border}`, color: active ? c : T.dim, borderRadius: 5, fontSize: "0.75rem", fontWeight: active ? 600 : 400, cursor: "pointer", transition: "all 0.15s", whiteSpace: "nowrap" as const }}>
      {children}
    </button>
  );
}
...
function ModelBadge({ mode, provider, modelId }: { mode: BuildMode; provider?: LlmProvider; modelId?: string }) {
  ...
  return (
    <span style={{ display: "inline-flex", alignItems: "center", gap: "0.3rem", fontSize: "0.62rem", fontWeight: 700, letterSpacing: "0.06em", textTransform: "uppercase", padding: "0.2rem 0.55rem", borderRadius: 5, background: isLlm ? `${meta?.color ?? "var(--gold)"}18` : "rgba(37,99,235,0.1)", border: `1px solid ${isLlm ? `${meta?.color ?? "var(--gold)"}50` : "rgba(37,99,235,0.25)"}`, color: isLlm ? (meta?.color ?? "var(--gold)") : "var(--color-info)", flexShrink: 0 }}>
      {isLlm ? (meta?.icon ?? "✦") : "◈"}{" "}
...
function SectionLabel({ children }: { children: React.ReactNode }) {
  return <div style={{ fontSize: "0.62rem", color: "rgba(180,83,9,0.5)", textTransform: "uppercase", letterSpacing: "0.1em", marginBottom: "0.6rem" }}>{children}</div>;
}

function Card({ children, highlight, style }: { children: React.ReactNode; highlight?: boolean; style?: React.CSSProperties }) {
  return (
    <div style={{ background: highlight ? "rgba(180,83,9,0.04)" : T.navy2, border: `1px solid ${highlight ? "rgba(180,83,9,0.25)" : T.border}`, borderRadius: 10, padding: "1.1rem 1.3rem", ...style }}>
      {children}
    </div>
  );
}
...
function ModeToggle({ mode, onChange, dataReady, llmReady }: { mode: BuildMode; onChange: (m: BuildMode) => void; dataReady: boolean; llmReady: boolean }) {
  ...
  return (
    <div style={{ display: "flex", gap: "0.6rem", marginBottom: "1.5rem" }}>
      {modes.map(m => (
        <button key={m.id} onClick={() => onChange(m.id)} style={{ flex: 1, textAlign: "left", padding: "0.85rem 1.1rem", background: mode === m.id ? "rgba(180,83,9,0.08)" : "var(--bg-subtle)", border: `1.5px solid ${mode === m.id ? "rgba(180,83,9,0.45)" : T.border}`, borderRadius: 9, cursor: "pointer", transition: "all 0.2s", display: "flex", gap: "0.85rem", alignItems: "flex-start" }}>
          <div style={{ fontSize: "1.2rem", color: mode === m.id ? T.gold : T.dim, marginTop: "0.05rem", flexShrink: 0, transition: "color 0.2s" }}>{m.icon}</div>
          <div>
            <div style={{ fontSize: "0.88rem", fontWeight: 700, color: mode === m.id ? T.gold : "var(--text-2)", marginBottom: "0.2rem", display: "flex", alignItems: "center", gap: "0.5rem" }}>
              {m.label}
              {m.id === "data" && dataReady && <span style={{ fontSize: "0.6rem", background: "rgba(21,128,61,0.15)", color: "var(--signal-bull)", border: "1px solid rgba(21,128,61,0.3)", borderRadius: 4, padding: "0.05rem 0.4rem" }}>ready</span>}
              {m.id === "llm"  && llmReady  && <span style={{ fontSize: "0.6rem", background: "rgba(21,128,61,0.15)", color: "var(--signal-bull)", border: "1px solid rgba(21,128,61,0.3)", borderRadius: 4, padding: "0.05rem 0.4rem" }}>ready</span>}
            </div>
            ...
            <div style={{ display: "flex", flexDirection: "column" as const, gap: "0.15rem" }}>
              {m.detail.map(d => <div key={d} style={{ fontSize: "0.65rem", color: mode === m.id ? "rgba(180,83,9,0.6)" : "var(--text-4)", display: "flex", gap: "0.35rem" }}><span style={{ opacity: 0.5 }}>–</span><span>{d}</span></div>)}
            </div>
          </div>
        </button>
      ))}
      ...
      {!strategy && (
        <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: "1rem", padding: "2rem 0" }}>
          {loading ? (<><Spinner /><p style={{ fontSize: "0.82rem", color: T.dim, margin: 0 }}>Analysing your preferences and current market conditions…</p></>) : (
            ...
            <button onClick={onGenerate} style={{ padding: "0.65rem 1.5rem", background: "rgba(180,83,9,0.15)", border: "1px solid rgba(180,83,9,0.4)", color: T.gold, borderRadius: 8, fontSize: "0.88rem", fontWeight: 600, cursor: "pointer" }}>✦ Generate strategy</button></>
          )}
        </div>
      )}
      ...
      {strategy && (
        <>
          <Card highlight>
            <div style={{ display: "flex", alignItems: "flex-start", gap: "1rem", marginBottom: "0.8rem" }}>
              ...
              <div style={{ flex: 1 }}>
                ...
                {strategy.macro_context && <div style={{ fontSize: "0.72rem", color: "var(--text-3)", lineHeight: 1.6, fontStyle: "italic", paddingTop: "0.4rem", borderTop: "1px solid var(--border)" }}><span style={{ color: "rgba(37,99,235,0.7)", fontStyle: "normal", fontWeight: 600 }}>Macro context: </span>{strategy.macro_context}</div>}
              </div>
            </div>
          </Card>
          ...
          {mode === "llm" && strategy?.macro_context && (
            <Card>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "0.6rem" }}>
                ...
                <div style={{ display: "flex", gap: "0.5rem" }}>
                  <span style={{ fontSize: "0.62rem", color: "rgba(21,128,61,0.7)", background: "rgba(21,128,61,0.08)", border: "1px solid rgba(21,128,61,0.2)", borderRadius: 4, padding: "0.15rem 0.5rem" }}>✦ Live events injected</span>
                  <span style={{ fontSize: "0.62rem", color: "rgba(37,99,235,0.6)", background: "rgba(37,99,235,0.08)", border: "1px solid rgba(37,99,235,0.2)", borderRadius: 4, padding: "0.15rem 0.5rem" }}>Model: {provider ?? "claude"} reasoning</span>
                </div>
              </div>
          ...
          <div>
            ...
            <div style={{ display: "flex", gap: "0.5rem", flexWrap: "wrap" }}>
              {(Object.keys(STYLE_META) as StyleFocus[]).map(s => (
                <button key={s} onClick={() => onUpdate({ ...strategy, style: s })} style={{ padding: "0.5rem 1rem", background: strategy.style === s ? `${STYLE_META[s].color}18` : "var(--border-subtle)", border: `1px solid ${strategy.style === s ? `${STYLE_META[s].color}60` : T.border}`, color: strategy.style === s ? STYLE_META[s].color : T.dim, borderRadius: 7, fontSize: "0.8rem", fontWeight: strategy.style === s ? 600 : 400, cursor: "pointer", transition: "all 0.15s", display: "flex", flexDirection: "column" as const, gap: "0.15rem", alignItems: "flex-start" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1.2rem" }}>
            ...
            <div><SectionLabel>Sector tilts (overweight)</SectionLabel><div style={{ display: "flex", gap: "0.4rem", flexWrap: "wrap" }}>{SECTORS.map(s => <PillToggle key={s} active={strategy.sector_tilts.includes(s)} onClick={() => onUpdate({ ...strategy, sector_tilts: strategy.sector_tilts.includes(s) ? strategy.sector_tilts.filter(x => x !== s) : [...strategy.sector_tilts, s] })} color="var(--signal-bull)">{s}</PillToggle>)}</div></div>
            <div><SectionLabel>Avoid sectors</SectionLabel><div style={{ display: "flex", gap: "0.4rem", flexWrap: "wrap" }}>{SECTORS.map(s => <PillToggle key={s} active={strategy.avoid_sectors.includes(s)} onClick={() => onUpdate({ ...strategy, avoid_sectors: strategy.avoid_sectors.includes(s) ? strategy.avoid_sectors.filter(x => x !== s) : [...strategy.avoid_sectors, s] })} color="var(--signal-bear)">{s}</PillToggle>)}</div></div>
          </div>
          ...
          <div style={{ display: "flex", justifyContent: "flex-end", gap: "0.75rem", paddingTop: "0.5rem" }}>
            ...
            <button onClick={onNext} style={{ padding: "0.55rem 1.4rem", background: T.gold, color: "var(--bg)", fontWeight: 700, borderRadius: 7, border: "none", fontSize: "0.85rem", cursor: "pointer" }}>Select themes →</button>
          </div>
        </>
      )}
    </div>
...
function Step2Themes({ themes, loading, mode, provider, modelId, onToggle, onBack, onNext }: { themes: RecommendedTheme[]; loading: boolean; mode: BuildMode; provider?: LlmProvider; modelId?: string; onToggle: (id: string) => void; onBack: () => void; onNext: () => void }) {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "1.2rem" }}>
      ...
      {themes.map(theme => (
        <button key={theme.id} onClick={() => onToggle(theme.id)} style={{ textAlign: "left", cursor: "pointer", padding: "1rem 1.2rem", background: theme.selected ? "rgba(180,83,9,0.06)" : T.navy2, border: `1px solid ${theme.selected ? "rgba(180,83,9,0.35)" : T.border}`, borderRadius: 10, transition: "all 0.2s", width: "100%" }}>
          <div style={{ display: "flex", alignItems: "flex-start", gap: "1rem" }}>
            <div style={{ width: 20, height: 20, borderRadius: 5, flexShrink: 0, marginTop: 2, background: theme.selected ? T.gold : "transparent", border: `1.5px solid ${theme.selected ? T.gold : "var(--border)"}`, display: "flex", alignItems: "center", justifyContent: "center", color: "var(--bg)", fontSize: "0.7rem", fontWeight: 900, transition: "all 0.15s" }}>{theme.selected && "✓"}</div>
            <div style={{ flex: 1 }}>
              <div style={{ display: "flex", alignItems: "center", gap: "0.6rem", marginBottom: "0.3rem" }}>
                <span style={{ fontWeight: 700, color: T.cream, fontSize: "0.92rem" }}>{theme.name}</span>
                <span style={{ fontSize: "0.62rem", padding: "0.1rem 0.45rem", borderRadius: 8, background: theme.momentum === "rising" ? "rgba(21,128,61,0.12)" : "var(--border)", color: theme.momentum === "rising" ? "var(--signal-bull)" : T.dim, border: `1px solid ${theme.momentum === "rising" ? "rgba(21,128,61,0.25)" : "var(--border)"}` }}>{theme.momentum ?? "stable"}</span>
              </div>
              ...
              {theme.conviction != null && (
                <div style={{ display: "flex", alignItems: "center", gap: "0.5rem", marginBottom: "0.4rem" }}>
                  ...
                  <div style={{ flex: 1, height: 4, background: "var(--border)", borderRadius: 2, overflow: "hidden" }}><div style={{ height: "100%", width: `${theme.conviction}%`, background: "rgba(180,83,9,0.7)", borderRadius: 2, transition: "width 0.4s" }} /></div>
                  <span style={{ fontSize: "0.68rem", color: T.gold, fontWeight: 600, minWidth: "2.5rem" }}>{theme.conviction}%</span>
                </div>
              )}
              <div style={{ fontSize: "0.72rem", color: "var(--signal-bull)", fontStyle: "italic" }}>✦ {theme.fit_reason}</div>
              {theme.is_llm_generated && mode === "llm" && <div style={{ fontSize: "0.6rem", color: "rgba(180,83,9,0.4)", marginTop: "0.2rem" }}>Claude-defined theme (not in your database)</div>}
            </div>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", paddingTop: "0.5rem" }}>
        ...
        <button onClick={onNext} disabled={selected.length === 0} style={{ padding: "0.55rem 1.4rem", background: T.gold, color: "var(--bg)", fontWeight: 700, borderRadius: 7, border: "none", fontSize: "0.85rem", cursor: selected.length === 0 ? "not-allowed" : "pointer", opacity: selected.length === 0 ? 0.4 : 1 }}>Allocate tickers →</button>
      </div>
    </div>
  );
}
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: "1.5rem" }}>
      <Card>
        ...
        <div style={{ marginTop: "0.75rem" }}>
          <div style={{ height: 5, background: "var(--border-subtle)", borderRadius: 3, overflow: "hidden" }}>
            <div style={{ height: "100%", borderRadius: 3, transition: "width 0.3s ease", width: `${Math.min(100, (totalWeight / investablePct) * 100)}%`, background: weightOk ? "linear-gradient(90deg, rgba(21,128,61,0.6), rgba(21,128,61,0.9))" : totalWeight > investablePct ? "linear-gradient(90deg, rgba(185,28,28,0.6), rgba(185,28,28,0.9))" : "linear-gradient(90deg, rgba(180,83,9,0.6), rgba(180,83,9,0.9))" }} />
          </div>
          <div style={{ display: "flex", justifyContent: "space-between", marginTop: "0.3rem" }}>
            ...
            <span style={{ fontSize: "0.62rem", color: weightOk ? "var(--signal-bull)" : "var(--signal-bear)", fontWeight: 600 }}>{!weightOk ? `⚠ ${(totalWeight - investablePct).toFixed(1)}% over limit — reduce BUY weights` : totalWeight < investablePct - 1 ? `${cashWeight.toFixed(1)}% stays as cash` : "✓ Fully allocated"}</span>
          </div>
        </div>
      </Card>
      ...
      <div style={{ background: T.navy2, border: `1px solid ${T.border}`, borderRadius: 10, overflow: "hidden" }}>
        ...
        {sortedTickers.map((t, idx) => {
          ...
          return (
            <div key={t.ticker} style={{ display: "grid", gridTemplateColumns: "2rem 2fr 1fr 1fr 1fr 1fr 2.5rem", padding: "0.6rem 0.85rem", alignItems: "center", borderBottom: idx < sortedTickers.length - 1 ? "1px solid var(--border-subtle)" : "none", background: !t.included ? "rgba(0,0,0,0.15)" : "transparent", opacity: t.included ? 1 : 0.4, transition: "all 0.15s" }}>
              <button onClick={() => onUpdate(t.ticker, "included", !t.included)} style={{ width: 18, height: 18, borderRadius: 4, background: t.included ? T.gold : "transparent", border: `1.5px solid ${t.included ? T.gold : "var(--border)"}`, cursor: "pointer", fontSize: "0.65rem", fontWeight: 900, color: "var(--bg)", display: "flex", alignItems: "center", justifyContent: "center" }}>{t.included && "✓"}</button>
              <div>
                <div style={{ display: "flex", alignItems: "center", gap: "0.4rem", flexWrap: "wrap" }}>
                  ...
                  {t.all_themes?.map((th: string) => <span key={th} style={{ fontSize: "0.58rem", color: "rgba(180,83,9,0.6)", background: "rgba(180,83,9,0.08)", border: "1px solid rgba(180,83,9,0.2)", borderRadius: 4, padding: "0.05rem 0.35rem" }}>{th}</span>)}
                  {mode === "data" && t.db_signal && <span style={{ fontSize: "0.58rem", fontWeight: 700, color: t.db_signal === "buy" ? "var(--signal-bull)" : t.db_signal === "watch" ? "var(--signal-neut)" : t.db_signal === "avoid" ? "var(--signal-bear)" : T.dim, background: t.db_signal === "buy" ? "rgba(21,128,61,0.1)" : t.db_signal === "watch" ? "rgba(180,83,9,0.1)" : "var(--border-subtle)", border: `1px solid ${t.db_signal === "buy" ? "rgba(21,128,61,0.25)" : t.db_signal === "watch" ? "rgba(180,83,9,0.25)" : "var(--border)"}`, padding: "0.05rem 0.35rem", borderRadius: 4, textTransform: "uppercase" }}>DB: {t.db_signal}</span>}
                </div>
                <div style={{ fontSize: "0.65rem", color: "var(--text-4)", marginTop: "0.1rem", lineHeight: 1.4 }}>{t.rationale}</div>
                {mode === "data" && (t.fundamental_score != null || t.technical_score != null) && <div style={{ display: "flex", gap: "0.6rem", marginTop: "0.35rem", flexWrap: "wrap" }}><SignalStrengthBar label="F" value={t.fundamental_score} color="rgba(180,83,9,0.8)" /><SignalStrengthBar label="T" value={t.technical_score} color="rgba(37,99,235,0.8)" /></div>}
                {mode === "data" && t.db_rationale && <div style={{ fontSize: "0.6rem", color: "var(--text-4)", marginTop: "0.25rem", lineHeight: 1.4, fontStyle: "italic" }}>Signal rationale: {t.db_rationale.slice(0, 100)}{t.db_rationale.length > 100 ? "…" : ""}</div>}
                {qty != null && t.editSignal === "BUY" && <div style={{ fontSize: "0.62rem", color: "var(--color-info)", marginTop: "0.15rem" }}>≈ {qty} units @ ${price?.toFixed(2)}</div>}
              </div>
              <div style={{ display: "flex", justifyContent: "center", gap: "0.3rem" }}>
                {(["BUY", "WATCH"] as const).map(sig => <button key={sig} onClick={() => onUpdate(t.ticker, "editSignal", sig)} style={{ padding: "0.18rem 0.5rem", background: t.editSignal === sig ? sig === "BUY" ? "rgba(21,128,61,0.15)" : "rgba(180,83,9,0.15)" : "transparent", border: `1px solid ${t.editSignal === sig ? sig === "BUY" ? "rgba(21,128,61,0.4)" : "rgba(180,83,9,0.4)" : "var(--border)"}`, color: t.editSignal === sig ? sig === "BUY" ? "var(--signal-bull)" : "var(--signal-neut)" : T.dimmest, borderRadius: 4, fontSize: "0.62rem", fontWeight: 700, cursor: "pointer", letterSpacing: "0.04em" }}>{sig}</button>)}
              </div>
              <div style={{ textAlign: "right" }}>
                {!isWatch ? <div style={{ display: "flex", alignItems: "center", gap: "0.2rem", justifyContent: "flex-end" }}><input type="number" value={t.editWeight} onChange={e => onUpdate(t.ticker, "editWeight", e.target.value)} style={{ width: 52, textAlign: "right", background: "var(--border-subtle)", border: "1px solid var(--border)", borderRadius: 4, color: T.cream, fontSize: "0.8rem", outline: "none", padding: "0.2rem 0.4rem" }} /><span style={{ fontSize: "0.65rem", color: T.dim }}>%</span></div> : <span style={{ fontSize: "0.72rem", color: T.dim }}>—</span>}
              </div>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", paddingTop: "0.5rem" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "0.75rem" }}>
          {!weightOk && buys.length > 0 && <span style={{ fontSize: "0.72rem", color: "var(--signal-bear)" }}>Weights must not exceed {investablePct}%</span>}
          {/* PATCH 8: updated button label */}
          <button onClick={onConfirm} disabled={committing || !weightOk} style={{ padding: "0.6rem 1.6rem", background: T.gold, color: "var(--bg)", fontWeight: 700, borderRadius: 7, border: "none", fontSize: "0.88rem", cursor: committing || !weightOk ? "not-allowed" : "pointer", opacity: committing || !weightOk ? 0.5 : 1 }}>
    ...
    <div style={{ maxWidth: 860, margin: "0 auto" }}>
      ...
      {!debugMode && !autoComplete && (loadingStrategy || loadingDataThemes || loadingDataAllocation || committing) && (
        <div style={{ padding: "2rem", textAlign: "center" }}>
          ...
          <div style={{ width: 180, height: 2, background: "var(--border)", borderRadius: 2, margin: "0 auto", overflow: "hidden" }}>
      ...
      {!debugMode && autoComplete && (
        <div style={{ padding: "2.5rem", textAlign: "center", display: "flex", flexDirection: "column", alignItems: "center", gap: "1rem" }}>
          ...
          <div style={{ fontSize: "0.78rem", color: "var(--text-3)", maxWidth: 380, lineHeight: 1.6 }}>
          ...
          <button onClick={() => router.push(`/dashboard/portfolio?portfolio_id=${portfolioId}&tab=recommendations`)}
            style={{ marginTop: "0.5rem", padding: "0.6rem 1.5rem", background: "rgba(180,83,9,0.15)", border: "1px solid rgba(180,83,9,0.4)", color: "var(--gold)", borderRadius: 6, cursor: "pointer", fontSize: "0.82rem", fontWeight: 600 }}>
            View recommendations →
          </button>
        </div>
      )}

      {error && <div style={{ background: "rgba(185,28,28,0.08)", border: "1px solid rgba(185,28,28,0.25)", borderRadius: 8, padding: "0.65rem 1rem", marginBottom: "1.2rem", fontSize: "0.8rem", color: "var(--signal-bear)" }}>{error}</div>}
      ...
      {step === 3 && debugMode && (
        <>
          ...
          {dataTickers.length > 0 && llmTickers.length > 0 && (
            <div style={{ background: "rgba(37,99,235,0.05)", border: "1px solid rgba(37,99,235,0.2)", borderRadius: 8, padding: "0.6rem 1rem", marginBottom: "1rem", display: "flex", alignItems: "center", gap: "1rem" }}>
              <span style={{ fontSize: "0.72rem", color: "rgba(37,99,235,0.8)" }}>◈ Both modes ready — switch tabs above to compare, then confirm the one you prefer.</span>
              <div style={{ marginLeft: "auto", display: "flex", gap: "0.5rem", fontSize: "0.7rem" }}>
                <span style={{ color: "var(--text-3)" }}>Data: {dataTickers.filter(t => t.included && t.editSignal === "BUY").length} BUY</span>
                <span style={{ color: "var(--text-4)" }}>·</span>
                <span style={{ color: "var(--text-3)" }}>LLM: {llmTickers.filter(t => t.included && t.editSignal === "BUY").length} BUY</span>
              </div>
            </div>
          )}
...
export default function PortfolioBuilderPage() {
  return (
    <Suspense fallback={<div style={{ color: "var(--text-4)", fontSize: "0.85rem", padding: "3rem 0" }}>Loading…</div>}>
