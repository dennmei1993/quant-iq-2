...
function classifyByRules(econ: Record<string, number | null>, macroAvg: number): Partial<RegimeClassification> {
  ...
  } else if (gdp != null && gdp < 1.5 && unemp != null && unemp > 4.5) {
    cycle_phase = "late";
  } else if (gdp != null && gdp < 1.5) {
    cycle_phase = "late";
  } else if (gdp != null && gdp > 3 && unemp != null && unemp < 4) {
    cycle_phase = "early";
  } else {
    cycle_phase = "mid";
  }
  ...
  if (gdp != null && gdp < 0) {
    growth_trajectory = "contracting";
  } else if (gdp != null && gdp < 1.5) {
    growth_trajectory = "decelerating";
  } else if (gdp != null && gdp > 3) {
    growth_trajectory = payrolls != null && payrolls > 200 ? "accelerating" : "stable";
  } else {
  ...
  if (cycle_phase === "recession" || (yieldInverted && sentimentWeak)) {
    risk_bias = "defensive";
  } else if (cycle_phase === "late" && sentimentWeak) {
    risk_bias = "risk-off";
  } else if (cycle_phase === "early" && monetary_stance === "accommodative") {
    risk_bias = "risk-on";
  } else {
    risk_bias = "neutral";
  }
...
Return ONLY valid JSON:
{
  ...
  "rationale": "2-3 sentences citing specific data: GDP, rates, inflation, and any geopolitical factors that influenced the classification",
  "confidence": 78,
  "key_indicators": {
    "gdp_growth": 0.7,
    "cpi_yoy": 2.87,
