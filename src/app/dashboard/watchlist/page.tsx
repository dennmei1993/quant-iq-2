...
interface Portfolio {
  id:       string
  name:     string
  universe: string[]   // always string[], never null
}
...
function WatchlistPanel({
  entries, loading, removing, onRemove, onBuy,
}: {
  ...
  if (!entries.length) return (
    <div style={{ color: 'var(--text-4)', fontSize: 'var(--fs-sm)', padding: '12px 0', lineHeight: 1.6 }}>
      No tickers yet. Use the panel on the right to add some.
    </div>
  )
  ...
  return (
    <div>
      <div style={{
        display: 'grid', gridTemplateColumns: '1fr 72px 72px 56px 48px 24px',
        gap: 6, padding: '0 0 6px', borderBottom: '1px solid var(--border)',
      ...
      {entries.map(e => (
        <div key={e.id} style={{
          display: 'grid', gridTemplateColumns: '1fr 72px 72px 56px 48px 24px',
          gap: 6, padding: '9px 0', borderBottom: '1px solid var(--border-subtle)', alignItems: 'center',
        }}>
          <div>
          ...
          <div style={{ textAlign: 'right' }}>
            {e.signal && <span style={{ fontSize: 9, fontWeight: 600, color: sigCol(e.signal), textTransform: 'uppercase' }}>{e.signal}</span>}
          </div>
          <button onClick={() => onBuy(e.ticker, e.name ?? null)}
          ...
          <button onClick={() => onRemove(e.ticker)} disabled={removing === e.ticker}
...
function SelectionPanel({
  universe, themes, watchedTickers, onAdd, adding,
}: {
  universe:       string[]
  themes:         Theme[]
  ...
  async function loadThemes() {
    ...
    try {
      ...
      setThemeGroups(
        themes
          .map(t => ({ id: t.id, name: t.name, momentum: t.momentum, tickers: (data.tickers ?? []).filter((tk: any) => tk.theme_id === t.id) }))
  ...
  const addBtn = (ticker: string, name: string | null) => {
    ...
    return (
      <button onClick={() => !on && handleAdd(ticker, name)} disabled={on || adding}
        style={{
          padding: '2px 10px', borderRadius: 'var(--r-sm)', flexShrink: 0,
          background: on ? 'none' : 'var(--text)',
          border: `1px solid ${on ? 'var(--border)' : 'var(--text)'}`,
          color: on ? 'var(--text-4)' : 'var(--bg)',
          fontSize: 'var(--fs-xs)', cursor: on ? 'default' : 'pointer', fontFamily: 'inherit',
        }}>{on ? '✓' : '+'}</button>
    )
  }
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
            ...
            {results.length > 0 && !selected && (
              <div style={{ position: 'absolute', top: '100%', left: 0, right: 0, zIndex: 50, background: 'var(--bg)', border: '1px solid var(--border)', borderRadius: 'var(--r-md)', marginTop: 2, overflow: 'hidden', boxShadow: '0 8px 24px rgba(0,0,0,0.1)' }}>
                {results.map((r, i) => {
                  ...
                  const showLbl = i === 0 || isTk !== (prev ? prev.ticker.toUpperCase().includes(q) : true)
                  return (
                    <div key={r.ticker}>
                      {showLbl && (
                      ...
                      <div onClick={() => { setSelected({ ticker: r.ticker, name: r.name }); setResults([]) }}
          ...
          {selected && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              ...
              <div style={{ display: 'flex', gap: 6 }}>
                ...
                <button onClick={() => handleAdd(selected.ticker, selected.name, note)}
                  disabled={adding || watchedTickers.has(selected.ticker)}
                  style={{
                    flex: 1, padding: '5px 12px', fontFamily: 'inherit',
                    ...
                    cursor: adding || watchedTickers.has(selected.ticker) ? 'default' : 'pointer',
                  }}>
      ...
      {tab === 'universe' && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          {!universe.length ? (
            <div style={{ fontSize: 'var(--fs-sm)', color: 'var(--text-4)' }}>No universe selected in portfolio settings.</div>
          ) : uniLoading ? (
            ...
            <>
              <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                ...
                <span style={{ marginLeft: 'auto', fontSize: 'var(--fs-xs)', color: 'var(--text-4)' }}>{universe.join(', ')}</span>
              </div>
...
export default function WatchlistPage() {
  ...
  const loadEntries = useCallback(async (portfolioId: string) => {
    setListLoad(true)
    try {
      ...
      const raw: WatchlistEntry[] = (
        data.watchlist ?? data.entries ?? data.items ?? data.data ??
      ...
      // Enrich with prices from daily_prices via /api/signals
      try {
        const tickers = raw.map(e => e.ticker).join(',')
        const sigRes  = await fetch(`/api/signals?tickers=${tickers}`)
        if (sigRes.ok) {
  ...
  useEffect(() => {
    async function load() {
      ...
      const [portRes, themeRes] = await Promise.all([
        supabase.from('portfolios').select('id, name, universe').eq('user_id', user.id).order('created_at', { ascending: true }),
        supabase.from('themes').select('id, name, momentum, conviction').eq('is_active', true).order('conviction', { ascending: false }).limit(8),
      ])

      const ports = portRes.data ?? []
      const raw   = (saved && ports.find(p => p.id === saved)) ? ports.find(p => p.id === saved)! : ports[0]

      const active: Portfolio | null = raw
        ? { id: raw.id, name: raw.name, universe: raw.universe ?? [] }
        : null
  ...
  async function handleBuy() {
    ...
    try {
      const res = await fetch('/api/portfolio/transaction', {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          portfolio_id: portfolio.id, ticker: txModal.ticker, type: 'buy',
          quantity: qty, price, fees: parseFloat(txFees) || 0,
          executed_at: new Date(txDate).toISOString(), notes: txNotes || undefined,
        }),
      })
      const d = await res.json()
      if (!res.ok) { setTxError(d.error ?? 'Failed'); return }
      setTxModal(null)
  ...
  function openBuyModal(ticker: string, name: string | null) {
  ...
  if (loading) return <div style={{ color: 'var(--text-4)', fontSize: 'var(--fs-sm)', padding: '2rem 0' }}>Loading…</div>
  if (!portfolio) return (
    <div style={{ color: 'var(--text-4)', fontSize: 'var(--fs-sm)' }}>
      No portfolio found.{' '}
      <button onClick={() => router.push('/dashboard/portfolio')} style={{ background: 'none', border: 'none', color: 'var(--accent)', cursor: 'pointer', fontSize: 'var(--fs-sm)' }}>Create one ↗</button>
    </div>
  )
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--sp-5)' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '30% 1fr', gap: 'var(--sp-5)', alignItems: 'start' }}>
        <div style={{ border: '1px solid var(--border)', borderRadius: 'var(--r-lg)', overflow: 'hidden' }}>
          ...
          <div style={{ padding: '4px 14px 14px' }}>
            <WatchlistPanel entries={entries} loading={listLoad} removing={removing} onRemove={handleRemove} onBuy={openBuyModal} />
          </div>
        </div>
        ...
        <div style={{ border: '1px solid var(--border)', borderRadius: 'var(--r-lg)' }}>
          ...
          <div style={{ padding: '14px', position: 'relative', minHeight: 320, maxHeight: '70vh', overflowY: 'auto' }}>
            <SelectionPanel universe={portfolio.universe} themes={themes} watchedTickers={watchedTickers} onAdd={handleAdd} adding={adding} />
          </div>
        </div>
      </div>
      ...
      {txModal && (
        <>
          ...
          <div onClick={e => e.stopPropagation()} style={{ position: 'fixed', top: '50%', left: '50%', transform: 'translate(-50%,-50%)', zIndex: 201, background: 'var(--bg)', border: '1px solid var(--border)', borderRadius: 8, padding: '1.4rem', width: 360, boxShadow: '0 16px 48px rgba(0,0,0,0.15)' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.6rem', marginBottom: '0.9rem' }}>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.5rem' }}>
                <div><div style={ls}>Quantity</div><input value={txQty} onChange={e => setTxQty(e.target.value)} placeholder="0" type="number" style={inputSt()} /></div>
                <div><div style={ls}>Buy price ($)</div><input value={txPrice} onChange={e => setTxPrice(e.target.value)} placeholder="0.00" type="number" style={inputSt()} /></div>
              </div>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.5rem' }}>
                <div><div style={ls}>Date</div><input value={txDate} onChange={e => setTxDate(e.target.value)} type="date" style={inputSt()} /></div>
                <div><div style={ls}>Fees ($)</div><input value={txFees} onChange={e => setTxFees(e.target.value)} placeholder="0.00" type="number" style={inputSt()} /></div>
              </div>
              <div><div style={ls}>Notes (optional)</div><input value={txNotes} onChange={e => setTxNotes(e.target.value)} placeholder="e.g. Earnings play" style={inputSt()} /></div>
            </div>
            {txQty && txPrice && (
            ...
            <div style={{ display: 'flex', gap: '0.5rem', justifyContent: 'flex-end' }}>
              <button onClick={() => setTxModal(null)} style={{ padding: '4px 12px', background: 'none', border: '1px solid var(--border)', borderRadius: 'var(--r-md)', color: 'var(--text-3)', fontSize: 'var(--fs-sm)', cursor: 'pointer', fontFamily: 'inherit' }}>Cancel</button>
              <button onClick={handleBuy} disabled={txSaving} style={{ padding: '4px 14px', background: 'rgba(21,128,61,0.1)', border: '1px solid rgba(21,128,61,0.3)', color: 'var(--signal-bull)', fontWeight: 600, borderRadius: 'var(--r-md)', cursor: txSaving ? 'not-allowed' : 'pointer', fontSize: 'var(--fs-sm)', opacity: txSaving ? 0.6 : 1, fontFamily: 'inherit' }}>
                {txSaving ? '…' : 'Record Buy'}
              </button>
            </div>
          </div>
